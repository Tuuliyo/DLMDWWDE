...
async def validate_transaction(
    ...
    with tracer.start_as_current_span("validate_transaction") as span:
        ...
        try:
            logger.info(f"Transaction received for validation: {transaction}")
            background_tasks.add_task(correct_transaction, transaction)
            logger.info(f"Background task added for transaction ID {transaction.transaction_id}")
            return {
                "status": "success",
            ...
            logger.error(f"Error while validating transaction ID {transaction.transaction_id}: {e}")
