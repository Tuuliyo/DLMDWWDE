...
class SolaceSourcePartition(StatelessSourcePartition):
    def __init__(self):
        # Initialize the messaging service and receiver
        self.messaging_service = self._initialize_messaging_service(POS_TRANSACTION_CONFIG)
    ...
    def next_batch(self):
        try:
            message = self.receiver.receive_message(timeout=1)  # Set an appropriate timeout
            if message:
